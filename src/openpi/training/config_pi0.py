...
class RLDSDroidDataConfig(DataConfigFactory):
    ...
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Make inputs look like they come from the DROID environment.
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        "pixels": "observation.images.top",
                        "gripper_position": "observation.state",
...
@dataclasses.dataclass(frozen=True)
class LeRobotSO100SingleDataConfig(DataConfigFactory):
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Make inputs look like they come from the SO100 single-arm environment
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
        ...
        # Convert images to uint8 numpy arrays, add masks
        data_transforms = _transforms.Group(
            inputs=[so100_policy_single.S0100SingleInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[so100_policy_single.S0100SingleOutputs()],
        )
        # Use delta actions for first 5 joints (not for gripper)
...
@dataclasses.dataclass(frozen=True)
class LeRobotWhiteForkDataConfig(DataConfigFactory):
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Make inputs look like they come from the SO100 single-arm environment
        # Map white_fork dataset cameras (left, front) to expected SO100 cameras
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        # Map dataset's left camera to main.left expected by policy
                        "observation/images.left": "observation.images.main.left",
                        # Map dataset's front camera to secondary_0 expected by policy  
                        "observation/images.front": "observation.images.secondary_0",
                        "observation/state": "observation.state",
                        "actions": "action",
                        "prompt": "prompt",
                    }
...
class LeRobotClearTrayDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[so100_policy.S0100Inputs(action_dim=model_config.action_dim)],
            outputs=[so100_policy.S0100Outputs()],
        )
...
_CONFIGS = [
    ...
    # Single-arm SO-100 configs by phospho.
    #
    TrainConfig(
        name="pi0_so100_single",
        model=pi0.Pi0Config(action_dim=6),  # 5 joints + 1 gripper
        data=LeRobotWhiteForkDataConfig(
            repo_id="JiabinQ/white_fork",  # Replace with your HuggingFace dataset
            base_config=DataConfig(
                local_files_only=False,  # Set to True for local-only datasets.
                prompt_from_task=True,
                action_sequence_keys=("action",),
    ...
    TrainConfig(
        name="pi0_fast_so100_single",
        model=pi0_fast.Pi0FASTConfig(action_dim=6, action_horizon=10),  # 5 joints + 1 gripper
        data=LeRobotSO100SingleDataConfig(
            repo_id="LegrandFrederic/dual-setup",  # You may want to change this to single-arm dataset
            base_config=DataConfig(
                local_files_only=False,  # Set to True for local-only datasets.
                prompt_from_task=True,
                action_sequence_keys=("action",),
